...
function QuizMode({ onBack }) {
  ...
  // Build 4 options: correct level + 3 distractors
  const options = card ? (() => {
    const others = LEVELS.filter(l => l.key !== card.level);
    const distractors = shuffle(others).slice(0, 3);
    return shuffle([level, ...distractors]);
  })() : [];
    ...
    <div style={{ minHeight: "100vh", background: BASE.bg, fontFamily: "'Cormorant Garamond', serif" }}>
      ...
      <div style={{ maxWidth: 640, margin: "80px auto", padding: "0 32px" }}>
        <div className="slide-up">
          ...
          <button className="btn-primary" style={{ marginTop: 32 }} onClick={() => setPhase("quiz")}>Begin Quiz →</button>
        </div>
      </div>
    </div>
  );
